...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_layer, cache_hidden_layer = fc_relu_forward(X, W1, b1)
        ...
        dx, dW1, db1 = fc_relu_backward(dx1, cache_hidden_layer)
        dW1 += self.reg * W1
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          layer[i], cache_layer[i] = fc_relu_forward(layer[i - 1],
                                                         self.params['W%d' % i],
                                                         self.params['b%d' % i])
        # Forward into last layer
        WLast = 'W%d' % self.num_layers
        bLast = 'b%d' % self.num_layers
        scores, cache_scores = fc_forward(layer[self.num_layers - 1],
                                              self.params[WLast],
                                              self.params[bLast])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        dx = {}
        dx[self.num_layers], grads[WLast], grads[bLast] = fc_backward(dscores, cache_scores)
        grads[WLast] += self.reg * self.params[WLast]

        # Backprop into remaining layers
        for i in reversed(range(1, self.num_layers)):
          # r = cache_layer[i + 1]
          dx[i], grads['W%d' % i], grads['b%d' % i] = fc_relu_backward(dx[i + 1], cache_layer[i])
